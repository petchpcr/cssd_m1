...
function getSql()
{
	...
	$WHERE_B_ID = "";
	if($B_ID != "0"){
		$WHERE_B_ID = " AND	washdetail.B_ID = $B_ID 
						AND wash.B_ID = $B_ID
...
if (isset($_POST["p_list_id"]) && isset($_POST["p_docno"])) {
	...
	$p_packing = "(SELECT sterile.PackingCode FROM sterile WHERE sterile.DocNo = '$p_docno' $WHERE_B_ID $limit)";
	$p_sterile = "(SELECT sterile.SterileCode FROM sterile WHERE sterile.DocNo = '$p_docno' $WHERE_B_ID $limit)";
	$p_approve = "(SELECT sterile.ApproveCode FROM sterile WHERE sterile.DocNo = '$p_docno' $WHERE_B_ID $limit)";
	$p_prepare = "(SELECT sterile.PrepareCode FROM sterile WHERE sterile.DocNo = '$p_docno' $WHERE_B_ID $limit)";
	...
	if($p_DB == 0){
		$sql_insert_stock_log = "	INSERT INTO itemstocklog ( 
									...
									SELECT 		s.ItemStockID, 
											$date, 
											(SELECT ID FROM steriledetail WHERE ItemStockID = s.ItemStockID $WHERE_B_ID ORDER BY ID DESC LIMIT 1)
											$VALUES_B_ID
									FROM  	(
											$sql_select_wash_item
										) AS s ";
		...
		$sql_insert_stock_log = "	INSERT INTO itemstocklog ( 
									...
									SELECT 		s.ItemStockID, 
									$date, 
											(SELECT TOP 1 ID FROM steriledetail WHERE ItemStockID = s.ItemStockID $WHERE_B_ID ORDER BY ID DESC)
											$VALUES_B_ID
									FROM  	(
											$sql_select_wash_item
										) s ";
	...
	if($p_DB == 0){
		$sql_update_item_stock =
									...
									SET 		itemstock.IsStatus = 2 , 
												...
												itemstock.LastSterileDetailID = (SELECT steriledetail.ID FROM steriledetail WHERE steriledetail.ItemStockID = itemstock.RowID $WHERE_B_ID ORDER BY ID DESC LIMIT 1) 
		...
		$sql_update_item_stock =
								"	UPDATE		itemstock 
									...
									SET 		itemstock.IsStatus = 2 , 
												...
												itemstock.LastSterileDetailID = (SELECT TOP 1 steriledetail.ID FROM steriledetail WHERE steriledetail.ItemStockID = itemstock.RowID $WHERE_B_ID ORDER BY ID DESC) 
	...
	while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
			...
			$sql_update_doc = 		"UPDATE 		wash 
									...
									AND 			(SELECT COUNT(*) FROM washdetail WHERE washdetail.WashDocNo = wash.`DocNo` AND washdetail.IsStatus < 2 $WHERE_B_ID) = 0 ";
	...
	if (isset($_POST["is_new"])){	
		...
		$WHERE_Sterile_B_ID = "";
		if($B_ID != "0"){
			$WHERE_Sterile_B_ID = "	AND     steriledetail.B_ID = '$B_ID'
		...
		while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
			...
			if(!$is_new){
				$sql_itemstockdetailbasket = " INSERT INTO itemstockdetailbasket (BasketID, ItemStockID, SSDetailID, WashDetailID, SterileDetailID, PairTime,IsActive $INSERT_B_ID) 
												VALUES('$BasketID', '$ItemStockID', '', '$ImportID', '$ID', '$date', 1 $INSERT_B_ID) ";
			}else{
